...
`;
    ...
    const USPList = [
        { icon: ICON_TRAIN, text: "Travel on any train on your selected date" },
        { icon: ICON_CLOCK, text: "Trains run every 15 mins*" },
  ...
  `;
    ...
    const customLog = (...args) => {
        ...
        console.log("%c" + args.join(" "), "background:#2a1f60;color:white;padding:2px 6px;");
    };
    ...
    const waitForElement = (selector, interval = 300, maxAttempts = 50) =>
        new Promise((resolve, reject) => {
            ...
            const check = () => {
                ...
                if (++attempt >= maxAttempts) return reject(`Element not found: ${selector}`);
    ...
    const addStyles = (css) => {
        if (!css || document.querySelector(".ccx-styles-hex-22-v1")) return;
