...
`;
  ...
  // Waits for an element by observing DOM mutations (no busy polling).
const waitForElement = (selector, timeout = 8000) => new Promise((resolve) => {
  ...
  const ensureContainerPlacement = () => {
    ...
    const heroCTA = document.querySelector("main.flex-1 > .max-w-limit button.bg-green");
...
const createAndAttachContainers = async () => {
  ...
  window.addEventListener("locationchange", () => {
    // keep watcher sentinels in sync
  ...
  // Dedup across multiple A/B reinits
