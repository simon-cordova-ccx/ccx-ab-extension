...
const NEW_IMAGE_URL = 'https://cdn-eu.dynamicyield.com/api/9880449/images/f35bcb377e67.jpg'
const TEST_NAME = "OZDE8 - Ambassador on LP";
const VARIATION = "VARIATION 1";
const CURRENT_URL = window.location.href;
const IS_STAGING_ENV = CURRENT_URL.includes('staging');
const ENVIRONMENT = IS_STAGING_ENV ? "staging" : "production";
const CLASS_STYLE = 'ccx-styles-ozde8-v1';
const CLASS_BODY = 'ccx-omaze-ozde8-v1';
const SELECTORS = {
  liveRentSellImage: 'live-rent-sell img',
};
...
// Main initialization function for OZDE8 experiment
const init = () => {
  try {
    ...
    waitForElements(
      [{ selector: SELECTORS.liveRentSellImage, count: 1 }],
      (results) => {
        console.log(results[0]?.elements);
        const image = results[0]?.elements[0];
        if (image) {
