...
const TEST_NAME = "OZ32 - Winner Testimonial in House LP Hero";
...
const styles = `
  ...
  /* Bottom container for image (trustpilot) */
  .ccx-slider-bottom {
    width: 100%;
    display: flex;
  ...
  .ccx-bottom-image { display:flex; align-items:center; justify-content:center; }
  .ccx-trustpilot { display:flex; align-items:center; justify-content:center; }
...
const fadeSwap = (container, updateFn) => {
  ...
  const onEnd = () => {
    ...
    updateFn();
    // next frame to trigger fade-in
    requestAnimationFrame(() => {
      container.classList.remove('is-fading-out');
      container.classList.add('is-fading-in');
      // clean up the helper class after transition
      const onInEnd = () => {
        container.removeEventListener('transitionend', onInEnd);
        container.classList.remove('is-fading-in');
      };
      container.addEventListener('transitionend', onInEnd);
    });
  };
...
function createCCXSlider(CONTROL_HMA_LAST_H3, SLIDE_DATA, ARROW_LEFT, ARROW_RIGHT, TRUST_PILOT_HTML, variation) {
  ...
  // Image or initials element (depends on variation)
  const img = document.createElement('img');
  img.alt = 'testimonial image';
  img.setAttribute("style", "max-width: 100% !important;");
  ...
  if (variation === 'new-control') wrapper.appendChild(bottom);
...
const removeOZ27Components = () => {
  const bodyTagHasOZ27Class = document.body.classList.contains('ccx-omaze-oz27-v1') || document.body.classList.contains('ccx-omaze-oz27-v2');
  if (!bodyTagHasOZ27Class) return;
  
  const slideShowWrapperV1 = document.querySelector('.ccx-omaze-oz27-v1 .slideshow-wrapper');
  if (slideShowWrapperV1) slideShowWrapperV1.remove();
  
  const slideShowWrapperV2 = document.querySelector('.ccx-omaze-oz27-v2 .slideshow-wrapper');
  if (slideShowWrapperV2) slideShowWrapperV2.remove();
  
  document.body.classList.remove('ccx-omaze-oz27-v1', 'ccx-omaze-oz27-v2');

  const ccx27Styles = document.querySelector('.ccx-styles-OZ27-v1');
  if (ccx27Styles) ccx27Styles.remove();
}
...
const handleSlideshowPosition = (CONTROL_HMA_LAST_H3) => {
  ...
  if (screenWidth >= 992) {
    // move slideshowWrapper after .home--banner .campaign-hero__content .yellow-btn
    ...
    // move slideShowWrapper after CONTROL_HMA_LAST_H3
...
const init = () => {
  try {
    ...
    waitForElements(
      ...
      function (results) {
        removeOZ27Components();

        addStyles(styles, VARIATION);

        document.body.classList.add('ccx-omaze-oz32-v1');
        customLog('[init] Added class ccx-omaze-oz32-v1 to body');

        const CONTROL_HMA_LAST_H3 = results[0].elements[0];
        if (!CONTROL_HMA_LAST_H3) return;
        
        customLog(CONTROL_HMA_LAST_H3);

        // Create CCX slider
        if (VARIATION === 'new-control') {
          createCCXSlider(
            CONTROL_HMA_LAST_H3,
            SLIDE_DATA_NEW_CONTROL,
            ARROW_LEFT,
            ARROW_RIGHT,
            TRUST_PILOT_HTML,
            VARIATION
