...
const subscriptionItems = [
    {
        ...
        button: {
            ...
            url: "https://omaze.co.uk/pages/surrey-ii"
        }
    },
    {
        ...
        description: "Your chance to win £1,000,000 a month.", // this may be 'in cash' not 'a month'
        button: {
            text: "Top up entries",
    ...
    },
    {
        ...
        description: "Your chance to win £100,000 a month",
        button: {
            text: "Draw Rules",
            url: "https://omaze.co.uk/pages/monthly-subscriber-cash-draw-experience-rules"
        }
    },
    {
        ...
        button: {
            text: "See this month\'s car",
            url: "https://omaze.co.uk/pages/yorkshire-iii-win-a-range-rover-hse"
        }
    },
    {
...
const applyVariationChanges = (price, variation, subscriptionFeatures) => {
    const ccxContainer = document.querySelector('.ccx-subs-container');
    ...
    if (variation !== "1" && variation !== "2") {
        customLog('Invalid variation, must be 1 or 2:', variation);
        return;
    }
    ...
    if (price !== '15' && price !== '25' && price !== '50') {
        customLog('Price ' + price + ' is not 15, 25 or 50.');
        // price = '25';
        return;
    }
    ...
    if (!subscriptionFeatures) {
        customLog('subscription-features element not provided');
        return;
    }

    // Create container
    const container = document.createElement('div');
    container.className = 'ccx-subs-container';
    ...
    if (!Array.isArray(subscriptionItems)) {
        customLog('subscriptionItems is not an array or is undefined');
        return;
    }

    subscriptionItems.forEach(item => {
        if (!item) {
            customLog('Invalid item in subscriptionItems');
            return;
        }
        ...
        if (item.icon) {
            ...
            customLog('Missing icon for item:', item.title || 'unknown item');
        }
...
function waitForElements(selectors, callback) {
    ...
    if (!window.DYO || !DYO.waitForElementAsync) {
        customLog('[waitForElements] DYO.waitForElementAsync is not available, attempting fallback with document.querySelector');
        ...
        const results = selectors.map(selector => {
            try {
                ...
                customLog('[waitForElements] Error querying ' + selector + ':', error);
                return [];
            }
        });
        ...
        if (results.every(result => result.length > 0)) {
            ...
            customLog('[waitForElements] Some elements not found via fallback:', results);
        }
        return;
    }
    ...
    const promises = selectors.map(selector =>
        DYO.waitForElementAsync(selector, 1, 200, 100) // 200ms interval, 100 retries = 20s
            ...
            .catch(error => {
                customLog('[waitForElements] Failed to find elements for ' + selector + ':', error);
    ...
    Promise.all(promises)
        .then(results => {
            ...
            if (results.every(result => result.length > 0)) {
                ...
                customLog('[waitForElements] Some selectors did not return elements:', results);
                ...
                const fallbackResults = selectors.map(selector => {
                    try {
                        ...
                        customLog('[waitForElements] Fallback error for ' + selector + ':', error);
                        return [];
                    }
                });
                ...
                if (fallbackResults.every(result => result.length > 0)) {
                    ...
                    customLog('[waitForElements] Some elements still not found via fallback:', fallbackResults);
                }
            }
        })
        .catch(error => {
            customLog('[waitForElements] Promise.all failed:', error);
        });
}
...
function observeSubscriptionManagementCards() {
    ...
    if (!targetElement) {
        customLog('[observeSubscriptionManagementCards] Target element #subscription-management__cards not found');
        return;
    }
...
function init() {
    try {
        ...
        document.body.classList.add('omaze-subs-25-callout-26-v1');
        customLog('[init] Added class omaze-subs-25-callout-26-v1 to body');
        ...
        waitForElements(elementSelectors, (results) => {
            ...
            if (tierCard && subscriptionFeatures) {
                ...
                customLog('tier-card or subscription-features element not found');
            }
        });
    } catch (error) {
