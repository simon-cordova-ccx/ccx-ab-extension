...
const styles = `
  #enter-now-material-tab-buttons-design [id*=subscription-tab-pane] .text-lg.font-bold {
    display: none !important;
  }
  #enter-now-material-tab-buttons-design [id*=single-purchase-tab-pane] > div:first-child.hide-on-plauer {
    display: none !important;
  }
  #enter-now-material-tab-buttons-design [id*=single-purchase-tab-pane] > div:first-child > img {
    display: none !important;
  }
  #enter-now-material-tab-buttons-design [id*=subscription-tab-pane] > div:first-child > div:first-child {
    display: none !important;
  }
  #enter-now-material-tab-buttons-design [id*=single-purchase-tab-pane] > div:first-child p {
    font-size: 14px;
  }
  #enter-now-material-tab-buttons-design [id*=single-purchase-tab-pane] > div:first-child span {
    font-family: Gellix;
    font-weight: 700;
    line-height: 100%;
    text-align: center;
...
const addStyles = (css) => {
  ...
  if (document.querySelector('.ccx-omaze-de2-v1')) {
  ...
  style.classList.add('ccx-omaze-de2-v1');
  ...
  if (aboveTrophyContainer) {
    ...
    if (paragraph) {
      ...
      if (isAlpen) {
        ...
        const aboveImageParagraph = document.querySelector('#enter-now-material-tab-buttons-design [id*=single-purchase-tab-pane] > div:first-child p');
        if (aboveImageParagraph) {
          aboveImageParagraph.innerHTML = `Mit einem Omaze-Abo sicherst du dir jeden Monat automatisch Lospakete mit dem besten <span>Preis-Leistungs-Verhältnis</span>`;
        }
        // Mit einem Omaze-Abo sicherst du dir jeden Monat automatisch Lospakete mit dem besten Preis-Leistungs-Verhältnis
      }

      if (!isAlpen) {
        // Dieses 2,5 Mio. € Haus am Plauer See plus 100.000€ Startkapital könnten bald dir gehören, während du die SOS - Kinderdörfer weltweit unterstützt.
        const longHeaderElement = document.querySelector('#enter-now-material-tab-buttons-design #nav-latest-alpen-haus-campaign > div:first-child > div.text-center p');
        if (longHeaderElement) {
          longHeaderElement.innerHTML = `Dieses 2,5 Mio. € Haus am Plauer See plus 100.000€ Startkapital könnten bald dir gehören, während du die&nbsp<b>SOS - Kinderdörfer weltweit&nbsp</b> unterstützt.</b>`;
        }
      }
...
const waitForElements = (elementSelector) => {
  ...
  DYO.Q(DYO.waitForElementAsync(elementSelector, 9, 10, 1000)).then(function (buttons) {
    ...
    if (window.location.href.includes('plauer')) {
      const element = document.querySelector('#enter-now-material-tab-buttons-design [id*=single-purchase-tab-pane] > div:first-child');
      if (element) {
        element.classList.add('hide-on-plauer');
      }
    }
...
async function init() {
  try {
    ...
    document.body.classList.add(TEST_NAME);
    customLog('[init] Added class ' + TEST_NAME + ' to body');
