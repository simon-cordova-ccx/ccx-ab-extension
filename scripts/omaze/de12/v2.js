...
.ccx-winningchance-summary {
  background: rgba(0, 144, 166, 0.05);
  margin-bottom: 1rem;
  font-size: 0.95rem;
  color: #333;
...
.ccx-winningchance-summary strong {
    line-height: normal;
}
...
`;
  ...
  const createGuaranteedWinnerComponent = (CONTROL_HOME_BANNER_CONTROLS) => {
    customLog('[createGuaranteedWinnerComponent] Creating guaranteed winner component...');

    const component = document.createElement('div');
    component.classList.add('ccx-banner-component');

    const guaranteedWinner = document.createElement('div');
    guaranteedWinner.classList.add('ccx-guaranteed-winner');
    guaranteedWinner.innerHTML = `
  ...
  `;
    ...
    component.appendChild(guaranteedWinner);
    CONTROL_HOME_BANNER_CONTROLS.insertAdjacentElement('afterend', component);

    customLog('[createGuaranteedWinnerComponent] Inserted guaranteed winner component.');
  };

  const createOddsComponent = () => {
    customLog('[createOddsComponent] Creating odds component...');

    const sliderWrapper = document.querySelector('.ccx-slider-wrapper');
    if (!sliderWrapper) {
      customLog('[createOddsComponent] No .ccx-slider-wrapper found — aborting.');
      return;
    }

    // Reuse existing odds if present; otherwise create it
    let odds = document.querySelector('.ccx-odds');
    if (!odds) {
      odds = document.createElement('div');
      odds.className = 'ccx-odds';
      odds.innerHTML = `
      <div class="ccx-odds__icon__wrapper">
    ...
    `;
      ...
      // Detach so we can move it cleanly
      odds.remove();
    }

    // Place directly before the slider
    sliderWrapper.insertAdjacentElement('beforebegin', odds);
    customLog('[createOddsComponent] Inserted .ccx-odds before .ccx-slider-wrapper.');
  };
  ...
  function createCCXSlider(CONTROL_HMA_LAST_H3, SLIDE_DATA, ARROW_LEFT, ARROW_RIGHT, TRUST_PILOT_HTML, variation) {
  ...
  `;
    ...
    // === BOTTOM (only for new-control) ===
    let bottom;
    // if (variation === 'new-control') {
    ...
    const render = () => {
      ...
      fadeSwap(main, () => {
        // Clear media container each time
        media.innerHTML = '';
    ...
    if (variation === 'new-control') {
      ...
      pDesc.textContent = first.name.replace(/,/g, '');
    } else if (variation === 'variation-1') {
      const initials = getInitials(first.name);
      initialsCircle.textContent = initials;
      media.appendChild(initialsCircle);
      ...
      <div class="ccx-winningchance-middle">
        ...
        <div class="ccx-winningchance-summary">
          <p>
            ...
            <strong>4.5&nbsp;mio Lose</strong>. Daraus ergaben sich die folgenden
  ...
  `;
    ...
    overlay.addEventListener('click', (e) => {
      // only close if the overlay itself was clicked (not children)
      if (e.target === overlay) {
        overlay.remove();
      }
    });
  ...
  const handleOddsPosition = () => {
    const oddsElement = document.querySelector('.ccx-odds');
    const imgElement = document.querySelector('.hero-mobile-addendum > .hma-content > img');
    const sliderWrapper = document.querySelector('.ccx-slider-wrapper');

    if (!oddsElement) {
      customLog('[handleOddsPosition] .ccx-odds not found');
      return;
    }
    ...
    if (screenWidth >= 992) {
      // On desktop: ensure the odds element is placed before the slider wrapper
      if (sliderWrapper && oddsElement.nextElementSibling !== sliderWrapper) {
        customLog('[handleOddsPosition] Moving .ccx-odds before .ccx-slider-wrapper (desktop)');
        sliderWrapper.parentNode.insertBefore(oddsElement, sliderWrapper);
      }
    } else {
      // On mobile: move odds element after the image
      if (imgElement && imgElement.nextElementSibling !== oddsElement) {
        customLog('[handleOddsPosition] Moving .ccx-odds after image (mobile)');
        imgElement.parentNode.insertBefore(oddsElement, imgElement.nextSibling);
      }
    }
  };
    ...
    try {
      ...
      waitForElements(
        ...
        (results) => {
          ...
          // Component Creation
          // ==========================
          createGuaranteedWinnerComponent(CONTROL_HOME_BANNER_CONTROLS);
          createOddsComponent();
          ...
          // ==========================
          // Attach click listener to .ccx-odds
          document.addEventListener('click', (e) => {
            const trigger = e.target.closest('.ccx-odds');
            if (trigger) {
              e.preventDefault();
              createCCXWinningChanceModal(CCX_MODAL_BG_IMAGE); // optional: pass bgImageUrl
            }
          });
